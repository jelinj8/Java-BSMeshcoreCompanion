...
public class GroupChatPane extends VBox {
    ...
    public GroupChatPane() {
        ...
        channelList.setCellFactory(lv -> new javafx.scene.control.ListCell<>() {
            @Override protected void updateItem(ChannelInfo item, boolean empty) {
                super.updateItem(item, empty);
                if (empty || item == null) { setText(null); return; }
                int unread = chatManager.unreadCountProperty(chatManager.channelKey(item)).get();
        ...
        chatManager.getChannels().addListener(
                (javafx.collections.ListChangeListener<ChannelInfo>) c -> channelList.refresh());
    }
    ...
    private HBox buildPasswordRow(PasswordField pf) {
        ...
        pf.textProperty().addListener((o, old, n) -> { if (!tf.getText().equals(n)) tf.setText(n); });
        tf.textProperty().addListener((o, old, n) -> { if (!pf.getText().equals(n)) pf.setText(n); });
    ...
    private void removeSelected(ListView<ChannelInfo> list) {
        ...
        confirm.showAndWait().ifPresent(btn -> {
            if (btn == ButtonType.OK) chatManager.removeChannel(selected);
        });
    }
}
