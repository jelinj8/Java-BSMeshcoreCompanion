...
public class DeviceRegistry {
	...
	private static final String KEY_TRANSPORT = "connection.device.%d.transport";
	...
	public static List<SavedDevice> load() {
		...
		for (int i = 0; i < count; i++) {
			...
			String transport = strProp(String.format(KEY_TRANSPORT, i), "usb");
			if (name != null && pubkey != null) {
				result.add(new SavedDevice(name, pubkey, portHint, transport));
			}
		}
		return result;
	}
